...
const getBooks= async function (req, res) {
   let bookYear= await BookModel.find( {year:2020} )  
   res.send({msg: bookYear})
}

const getParticularBooks= async function (req, res) {
    let particularBook= await BookModel.find({$or: [ {bookName : {$regex:req.params.key}}]})
...
const getXINRBooks= async function (req, res) {
    let INRBooks= await BookModel.find({"prices.indianPrice": { $in: [ "Rs.100", "Rs.200", "Rs.500"] } })
